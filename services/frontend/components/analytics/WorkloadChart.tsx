...
// Ported VERBATIM from OLD app/features/analytics/components/WorkloadChart.tsx.
// recharts config + color scale copied unchanged.
      ...
      <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {data.openTaskCount === 0
            ? 'No tasks'
            : data.openTaskCount <= 3
            ? 'Light workload'
            : data.openTaskCount <= 7
            ? 'Medium workload'
            : data.openTaskCount <= 10
            ? 'Heavy workload'
            : 'Overloaded'}
        </p>
      </div>
...
export const WorkloadChart = ({ data, isLoading }: WorkloadChartProps) => {
  ...
  const formattedData = sortedData.map((item) => ({
    ...item,
    displayName:
      item.userName.length > 15 ? item.userName.substring(0, 12) + '...' : item.userName,
  }));
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={formattedData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
            ...
            <Bar dataKey="openTaskCount" radius={[8, 8, 0, 0]}>
              {formattedData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={getWorkloadColor(entry.openTaskCount)} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
