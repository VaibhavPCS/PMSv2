...
export const usePermissions = (): PermissionConfig => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // HTTP-only cookies are sent automatically with getRequest
        const response = await getRequest<any>('/auth/me');
  ...
  return useMemo(() => {
    ...
    const workspaceRole =
      userData.workspaces?.find((w: any) => w?.workspaceId?._id === currentWorkspaceId)?.role || 'member';
