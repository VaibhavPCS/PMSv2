...
const _publishStatusChangedWithRetry = async (taskId, from, to, userId) => {
  ...
  for (let attempt = 0; attempt < delays.length; attempt += 1) {
    try {
      ...
      await PublishTaskStatusChanged(taskId, from, to, userId);
      return;
    } catch (err) {
      lastError = err;
      logger.error(`[TASK_STATUS_PUBLISH_FAILED] attempt=${attempt + 1} taskId=${taskId} from=${from} to=${to} userId=${userId} error=${err.message}`);
    }
  }
...
const UpdateStatus = async (taskId, { status, reason }, userId) => {
  const { updated, previousStatus } = await prisma.$transaction(async (tx) => {
    ...
    return { updated: updatedTask, previousStatus: task.status };
  });

  await _publishStatusChangedWithRetry(taskId, previousStatus, status, userId);
  return updated;
};
...
const ApproveTask = async (taskId, { comment }, userId) => {
  ...
  await _publishStatusChangedWithRetry(taskId, previousStatus, TASK_STATUS.APPROVED, userId);
  return updated;
};
...
const RejectTask = async (taskId, { reason, rejectTo }, userId) => {
  ...
  await _publishStatusChangedWithRetry(taskId, previousStatus, TASK_STATUS.REJECTED, userId);
...
const HandoverTask = async (taskId, { notes, handoverTo }, userId) => {
  ...
  await _publishStatusChangedWithRetry(taskId, previousStatus, TASK_STATUS.IN_REVIEW, userId);
  return updated;
};
