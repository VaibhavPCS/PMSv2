...
process.env.DB_NAME = process.env.DB_NAME || 'pms_comment';
if (!process.env.DATABASE_URL) {
  const dbBase = process.env.DB_BASE?.trim();
  const dbName = process.env.DB_NAME?.trim();

  if (!dbBase || !dbName) {
    throw new Error('Missing required DB_BASE or DB_NAME for DATABASE_URL composition in comment-service.');
  }
