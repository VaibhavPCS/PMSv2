...
const rawKey = process.env.COMMENT_ENCRYPTION_KEY;

if (!rawKey || !/^[0-9a-fA-F]{64}$/.test(rawKey)) {
    throw new Error('COMMENT_ENCRYPTION_KEY must be a 64-character hex string.');
}
...
const Encrypt = (plaintext) => {
    ...
    try {
        ...
        throw new Error(`Failed to encrypt comment: ${err.message}`);
    }
};
